...
// Special keys
extern const std::string kIdKey;
extern const std::string kTypeKey;
extern const std::string kTimeKey;
...
class Item : public dynamic {
 public:
  ...
  id_t id() const {
    try {
      return at(QueryConstants::kIdKey).get<int64_t>();
  ...
  int64_t ts() const {
    try {
      return at(QueryConstants::kTimeKey).get<int64_t>();
    } catch (const std::exception& ex) {
      LOG_EVERY_N(WARNING, 1000) << ex.what();
    }
  }
...
class ItemOptimized : public Item {
 public:
  ...
  ItemOptimized() : Item(), id_(0), ts_(0) {}
  ...
  void syncIdTs() {
    try {
      id_ = at(QueryConstants::kIdKey).get<int64_t>();
      ts_ = at(QueryConstants::kTimeKey).get<int64_t>();
    } catch (const std::exception& ex) {
      LOG_EVERY_N(WARNING, 1000) << ex.what();
    }
  }
  ...
  void syncMap() {
    try {
      (*this)[QueryConstants::kIdKey] = dynamic(int64_t(id_));
      (*this)[QueryConstants::kTimeKey] = dynamic(int64_t(ts_));
    } catch (const std::exception& ex) {
      LOG_EVERY_N(WARNING, 1000) << ex.what();
    }
  }
